...
public class PaymentProcessor {
  ...
  public void createPayments(String paymentId, String merchantId, double startingBalance) {
    if(merchants.containsKey(merchantId) && startingBalance >= 0 && !payments.containsKey(paymentId)) {
      payments.put(paymentId, new PaymentIntent(paymentId, merchantId, startingBalance));
    }
  }
  ...
  public void succeed(String paymentId) {
    ...
    if(paymentIntent != null && Objects.equals(paymentIntent.state, State.PROCESSING)) {
      paymentIntent.state = State.SUCCEEDED;
  ...
  public void update(String paymentId, double newAmount) {
    PaymentIntent paymentIntent = payments.get(paymentId);
    if(paymentIntent != null && paymentIntent.state == State.REQUIRES_ACTION && newAmount >= 0) {
